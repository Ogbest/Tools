...
namespace ProtoBuf.Meta
{
    ...
    public abstract partial class TypeModel
    {
        ...
        {
            int key = GetKey(ref type);
            //UnityEngine.Debug.LogError($"DeserializeCore {key >= 0} && {!Helpers.IsEnum(type)}");
        ...
        internal static Type GetListItemType(TypeModel model, Type listType)
        {
            ...
            if (listType.FullName == "System.String") return null;//Mark  Proto兼容修改
#endif

#if !SERVER
            if (listType is ILRuntimeWrapperType)
            {
                //UnityEngine.Debug.Log($"ReflectionType :{((ILRuntimeWrapperType)listType).CLRType.GenericArguments[0].Value.ReflectionType == null}");
